...
def compared_grouped(df_base, df_new, by, target_columns, diff_columns_suffixe='DIFF'):
    """
    ...
    def diff(column):
        return lambda df: (df[column] - df[column + '_new']).abs()

    return df_base.merge(df_new, how='outer', validate='one_to_one', suffixes=('', '_new')).assign(
        **{
